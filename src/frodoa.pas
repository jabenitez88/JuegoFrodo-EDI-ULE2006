...
{ Simple procedure que pide insertar los datos, recibe por variable las filas (N de tipo longint)
las columnas(M de tipo longint) y la energía (e de tipo integer) }

PROCEDURE pedirDatosPrimarios(var N:longint; var M:longint;var e:tEnergia);
Begin
	repeat
	...
	until(M<=500) and (M>0);
        ...
        repeat
        	writeln('Introduzca la energía inicial de frodo: (MENOR A ', N*M, ')');
        	readln(e);
        until(e<=N*M);
End;
...
PROCEDURE pedirContenidoCasillas(N:longint; M:longint; var tablero:tTablero);
...
Begin	
	...
	writeln('OBJETOS => 0 - NADA | 1 - PAN | 2 - FRUTOS | 3 - ORCO');
        ...
        for i:=1 to N do
        	for j:=1 to M do
                        begin
                        	writeln('Introduzca la direccion para la casilla [',i,',',j,']: ');
                        	readln(tablero[i,j].movimiento);
                        	writeln('Introduzca el objeto [',i,',',j,']: ');
...
ya que se modifica, el número de filas, el número de columnas (N y M de tipo longint pasados por valor) y 
el estado de frodo ( de tipo tEstado pasado por valor), ya que el movimiento no se ejecuta de la misma 
manera. }

PROCEDURE cambiaMovSiEsBorde(cf: tCasillaFrodo; var mov: string; N: longint; M: longint; est : tEstado);
...
Los parámetros que recibe por valor son mv de tipo tMovimiento ( word ), est (tEstado) N ( longint )
M ( longint ) y por variable cf de tipo tCasillaFrodo. }

PROCEDURE moverAFrodo(mv: tMovimiento; est: tEstado ; var cf: tCasillaFrodo; N : longint; M: longint);
...
FUNCTION  esFinDePartida(e : tEnergia; est: tEstado; ob: tObjeto; cf: tCasillaFrodo; N: longint; M: longint):boolean;
