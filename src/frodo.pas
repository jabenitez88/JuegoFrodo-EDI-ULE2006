...
tipo tTiempoMareo (un longint) la cual modificará dependiendo del objeto con el que se encuentre 
...
o está normal, le llega por valor la variable t de tipo tTiempoMareo (longint) por la cual, si su valor
...
las filas ( longint ) y las columnas ( longint ) , estas 2 últimas por valor.}

PROCEDURE borrarObjeto (var tab: tTablero; fil : longint; col : longint);
...
las columnas(M de tipo word) y la energía (e de tipo longint) }
...
Begin
        	...
        	writeln('Introduzca el numero de columnas: ');
        	readln(M);

        	writeln('Introduzca la energía inicial de frodo: ');
        	readln(e);
End;
	...
	if (est = NORMAL) then begin
				...
				'N' :  // Movimiento al NORTE ...
					...
					if (cf.fila = 1) // ... ¡ en la PRIMERA FILA !
							then begin
								if (cf.columna = M) AND (N <> 1) 
									then mov:= 'SUR'      // Casilla [1, MAX]
								else if (cf.columna = M) AND (N = 1) 
									then mov:= 'OESTE'      // Casilla [1, MAX]
				...
				'S' : // Movimiento al SUR ...
					...
					if (cf.fila = N)  // ... ¡ en la ÚLTIMA FILA !
							then begin
								if (cf.columna = 1) AND (N <> 1)
									then mov:= 'NORTE'    // Casilla [MAX, 1]
								else if (cf.columna = 1) AND (N = 1)	
									then mov:= 'ESTE'    // Casilla [MAX, 1]
				...
				'E' : // Movimiento al ESTE ...
					...
					if (cf.columna = M) // ... ¡ en la ÚLTIMA COLUMNA !
						then begin
							if (cf.fila = N) AND (M <> 1)
								then mov:= 'OESTE'   // Casilla [MAX, MAX]
							else if (cf.fila = N) AND (M = 1)
								then mov:= 'NORTE'   // Casilla [MAX, MAX]
				...
				'O' : // Movimiento al OESTE ...
					...
					if (cf.columna = 1) // ... ¡ en la ÚLTIMA COLUMNA !
						then begin
							if (cf.fila = 1) AND (M <> 1)  
								then mov:= 'ESTE'    // Casilla [1, 1]
							else if (cf.fila = 1) AND (M = 1)  
								then mov:= 'SUR'    // Casilla [1, 1]
...
N ( longint ) y M ( longint ), devuelve un booleano. }
...
{ Esta función nos sirve para poder hacer que las filas, las columnas y la energía se 
devuelvan como string en lugar de ser word, esto nos servirá para poder crear el
mensaje que meteremos en el recorrido.txt 

Recibe un entero de tipo longint y devuelve un string. }

FUNCTION converNum(entero : longint):string;
...
if ((verObjeto(tablero[1,1].objeto) <> 'NADA') or (verObjeto(tablero[filas,columnas].objeto) <> 'NADA'))
	then begin
		...
		// Primero comprueba si el tablero es 1x1
		if (filas = 1) AND (columnas = 1)
			then begin 
				...
				writeln('Frodo ha llegado a Mordor y ha destruído el anillo');
				mensaje:= '[1,1] - ' + verMovimiento(movimientoActual) + ' - ' + converNum(frodo.energia);
				escribeLog(ficheroLog,mensaje);
				end;
				
		// Si no es 1x1 entra en el bucle repeat y comienza a moverse
		if (filas<>1) OR (columnas<>1) then
		begin
		repeat
			
			// Movimiento, objeto y estado de la casilla actual de Frodo
			movimientoActual := tablero[frodo.casilla.fila, frodo.casilla.columna].movimiento;
			objetoActual := tablero[frodo.casilla.fila, frodo.casilla.columna].objeto;
			...
			//Si la energía de Frodo es distinta de 0, frodo se mueve.
			if (frodo.energia<>0)
			...
			// Inicializamos la variable mensaje con el texto que imprimirá en el log
			...
			// Comprobamos si findepartida es True o False
			finPartida := esFinDePartida(frodo.energia,estadoActual,objetoActual,frodo.casilla,filas,columnas);
               
		until(finPartida = TRUE); // Si finPartida es true, sale del bucle y finaliza.

		// Si Frodo llega a Mordor escribe la última línea [N,M] - Dirección - Energía
		if (frodo.casilla.fila = filas) AND (frodo.casilla.columna = columnas) 
				then begin	
					mensaje:= '[' + converNum(frodo.casilla.fila) + ',' + converNum(frodo.casilla.columna) + '] - ' + verMovimiento(tablero[frodo.casilla.fila, frodo.casilla.columna].movimiento) + ' - ' + converNum(frodo.energia);
					escribeLog(ficheroLog, mensaje);
					end;
